...
export default function Login(props) {
  ...
  const depositListHandler = () => {
    console.log('Calling depositListHandler with viewId:', viewId);
    //if(!viewId) return;
    setLoaderDeposit(true);
    axios({
      method: "GET",
      url: Apiconfigs.transactionUsersList,
      headers: {
  ...
  return (
    <Box className={classes.LoginBox} mb={5} mt={20}>
                    ...
                    {depositList &&
                      depositList?.map((row, index) => (
                        <TableRow className={classes.tbody} key={row.coinName}>
                          ...
                          <TableCell style={{ color: "black" }} align="Center">
                            {sortAddress(row?.fromAddress)}{row.userId.userName}
                          </TableCell>
                          <TableCell style={{ color: "black" }} align="Center">
                            {sortAddress(row?.recipientAddress)}{row.toDonationUser.userName}
                          </TableCell>
                          <TableCell style={{ color: "black" }} align="Center">
                            {row?.transactionType}
                          </TableCell>
